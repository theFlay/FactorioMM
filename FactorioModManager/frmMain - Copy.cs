...
namespace FactorioModManager
{
    public partial class frmMain : Form
    {
        ...
        private void SearchForModInfo()
        {
            ...
            string modData = webClient.DownloadString("/api/mods?q=" + currentMod.name);
            ...
            foreach (var item in searchDataJson.results)
            {
                if (item.name.ToString() == currentMod.name)
                {
                    ...
                    currentModFilename = item.latest_release.file_name.ToString();
                    currentModUrl = item.latest_release.download_url.ToString();
                    LogWriteLine(" done.");
